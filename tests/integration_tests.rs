...
impl CommandResult {
    fn assert_success(&self) {
        if self.status != 0 {
            panic!("Command failed with status {}: stderr: {}", self.status, self.stderr);
        }
    }
    
    fn assert_failure(&self) {
        // For LDR, failure is indicated by error messages in stdout, not exit code
        if !self.stdout.contains("Invalid") && !self.stderr.contains("Error") && !self.stdout.is_empty() {
...
mod subtask_operations {
    ...
    fn test_add_subtask() {
        ...
        result.assert_success();
        assert!(result.stdout.contains("✓ Added subtask to task 1: Subtask A"));
...
mod filtering_tests {
    ...
    fn test_filtering_with_limits() {
        ...
        result.assert_success();
        let matching_lines: Vec<&str> = result.stdout.lines()
    ...
    fn test_filtering_no_matches() {
        ...
        result.assert_success();
        assert!(result.stdout.contains("No items found matching filter: \"nonexistent\""));
    }
}
